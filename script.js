...
function getZipLib() {
  const lib = globalThis.zip || globalThis.zipjs;
  if (!lib || !lib.ZipReader || !lib.BlobReader || !lib.BlobWriter) {
    throw new Error("Library ZIP gagal dimuat. Pastikan koneksi internet tersedia untuk memuat zip.js.");
  }
  return lib;
}
...
async function readZip(file, password = undefined) {
  ...
  const reader = new zipLib.ZipReader(
    new zipLib.BlobReader(file),
...
async function extractZip(file) {
  ...
  try {
    for (const entry of entries) {
      ...
      let blob;
      try {
        blob = await entry.getData(new (getZipLib().BlobWriter)(), {
          password: password,
...
async function extractZipWithPassword(file, password) {
  ...
  try {
    for (const entry of entries) {
      if (entry.directory) continue;
      const blob = await entry.getData(new (getZipLib().BlobWriter)(), {
        password,
